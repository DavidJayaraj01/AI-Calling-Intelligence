...
class ActionItemGenerator:
    ...
    def _load_model(self):
        ...
        try:
            if not self.openai_api_key or self.openai_api_key == "your-openai-api-key-here":
                logger.warning("OpenAI API key not configured. Action item generation will use fallback methods.")
            else:
                logger.info("Action item generator initialized with OpenAI API")
        except Exception as e:
            logger.error(f"Error initializing OpenAI connection: {e}")
            logger.info("OpenAI connection will be established on first use")
    ...
    async def generate_action_items(
        ...
        try:
            ...
            action_items = await self._generate_with_openai(context)
    ...
    async def _generate_with_openai(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Use OpenAI API to generate action items"""
        try:
            if not self.openai_api_key or self.openai_api_key == "your-openai-api-key-here":
                logger.warning("OpenAI API key not configured, using fallback method")
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    json={
                        "model": "gpt-3.5-turbo",
                        "messages": [
                            {"role": "system", "content": "You are an AI assistant that generates actionable follow-up tasks based on call transcripts. Always respond with valid JSON only."},
            ...
            logger.error(f"Error generating action items with OpenAI: {e}")
            return self._generate_fallback_action_items(context)
